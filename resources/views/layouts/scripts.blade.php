...
<script type="text/javascript">
    ...
    $(document).ready(function($){
        ...
        $("input[type='cnpj']").mask('00.000.000/0000-00');
    });

    //Funçoes Clonar
    function clonar(args){
    var i = $(`${args}`).length + 1;
    var clonable = $(`${args}:first`).clone();
      clonable.find("input").attr('id', function(idx, attrVal) {
        return attrVal + i;
      })
      clonable.find("input").attr('name', function(idx, attrVal) {
        return attrVal.replace('[1]', `[${i}]`);
      });
      clonable.find("input").val('');
      clonable.insertAfter(`${args}:last`);
    }
    function rmclone(args) {
        if($(`${args}`).length > 1){
          $(`${args}:last`).remove();
        }
    }

    //Validaçoes de Formulario
    $("form").on('submit', function(e) {
    $("form").valid();
    var ele = $("form :input.error:first");
    if (ele.is(':hidden')) {
      $('.nav-link').removeClass('active');
      $('.tab-pane').removeClass('active show');
      
      $(`#${ele.closest('.tab-pane').attr('id')}-tab`).addClass('active');
      $(`#${ele.closest('.tab-pane').attr('id')}`).addClass('active show');
    }
  });
